...
export function verifyCommand(
  ...
  if (!hasGivenOptions) {
    if (limits.includes(command)) {
      return new Error(
        `Expected an option object along with the command: '${command}'`,
      );
    }
  }
  ...
  if (commandRequiresOptions) {
    ...
    for (const option of allRequiredOptions) {
      if (!givenOptionKeys.includes(option)) {
        return new Error(
          `expected '${option}' in options but recieved undefined`,
        );
      }
    }
  }
  ...
  if (hasGivenOptions) {
    ...
    for (const [key, value] of Object.entries(givenOptions)) {
      ...
      const hasMinMax =
        optionLimitKeys.includes('min') || optionLimitKeys.includes('max');

      // If we dont have an array (enum) or a min max property, something is not right
      if (optionsLimits === undefined) {
        return new Error(
          `Unexpected error with following parameters: ${[
            command,
            key,
            `[${Object.entries(givenOptions)}]`,
          ]}, no option limits found`,
        );
      }
      ...
      if (hasMinMax) {
        ...
        if (hasExceeded || hasDeceed) {
          return new Error(
            `invalid value ${key}: ${value}, expected ${key} to range between ${min}, ${max}`,
          );
        }
      }
      ...
      if (commandValueIsEnum && !optionsLimits.includes(value)) {
        return new Error(
          `invalid value ${key}: ${value} expected [${currentCommandLimits.toString()}]`,
        );
      }
    }
  }
  ...
  const reducer = (finalDataObject: object, nextDataPoint: string): object => {
    ...
    if (isCommaSeperated) {
      formattedValue = value
        .split(',')
        .map(val => (Number.isNaN(parseFloat(val)) ? val : parseFloat(val)));
    } else {
      formattedValue = Number.isNaN(parseFloat(value))
        ? value
        : parseFloat(value);
    }
    ...
    return {
      ...finalDataObject,
      [key]: formattedValue === undefined ? value : formattedValue,
    };
  };
